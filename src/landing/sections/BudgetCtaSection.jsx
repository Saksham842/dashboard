...
export const BudgetCtaSection = () => {
  ...
  const numStyle = { fontFamily:'Sora,sans-serif', fontSize:'clamp(28px,3.5vw,40px)', fontWeight:700, color:T.gold, lineHeight:1.1 };
  const suffixStyle = { fontFamily:'Sora,sans-serif', fontSize:'clamp(14px,1.8vw,20px)', fontWeight:400, color:T.gold, opacity:0.6 };
  ...
  return (
    <section ref={ref} data-scroll style={{ background:T.bg, padding:'90px clamp(16px,4vw,32px)', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ maxWidth:900, margin:'0 auto', position:'relative', zIndex:2, textAlign:'center', display:'flex', flexDirection:'column', alignItems:'center', gap:0 }}>
        ...
        {/* ── Eyebrow ── */}
        <div style={{ fontFamily:'Inter,sans-serif', fontSize:12, fontWeight:600, letterSpacing:'0.22em', color:T.gold, marginBottom:16, ...fade(visible,0) }}>
          — YOUR HIRING BUDGET, RECALCULATED —
        </div>

        {/* ── Heading ── */}
        <h2 style={{ fontFamily:'Sora,sans-serif', fontSize:'clamp(1.6rem,3.8vw,2.8rem)', fontWeight:700, color:T.cream, letterSpacing:'-0.02em', lineHeight:1.25, margin:'0 0 8px', ...fade(visible,0.1) }}>
          What Lina costs you now.
        </h2>
        <div style={{ fontFamily:'Sora,sans-serif', fontSize:'clamp(1.4rem,3.2vw,2.2rem)', fontWeight:600, color:T.pink, letterSpacing:'-0.02em', lineHeight:1.25, marginBottom:24, ...fade(visible,0.2) }}>
          Spoiler: less than one recruiter&apos;s salary.
        </div>

        {/* ── Subheading ── */}
        <p style={{ fontFamily:'Inter,sans-serif', fontSize:15, color:T.muted, lineHeight:1.6, margin:'0 0 36px', maxWidth:560, ...fade(visible,0.25) }}>
          ...
          {STATS.map((s,i) => (
            <React.Fragment key={i}>
              <div style={{ flex:1, textAlign:'center', padding:'4px 0' }}>
                ...
                <div style={{ fontFamily:'Inter,sans-serif', fontSize:12, color:T.muted, marginTop:4, letterSpacing:'0.02em' }}>
                  {s.label}
                </div>
              </div>
              {i < STATS.length - 1 && (
