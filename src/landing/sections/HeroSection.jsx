  ...
  return (
    <section ref={ref} style={{
      ...
      <div data-scroll style={{ position: 'relative', zIndex: 3, maxWidth: 620, ...contentStyle }}>
        <h1 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1.08, marginBottom: 'clamp(40px, 6vh, 56px)', color: '#F5F0E8' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0 14px', fontSize: isMobile ? 'clamp(2.2rem, 7vw, 3.2rem)' : 'clamp(2.8rem, 9vh, 5rem)' }}>
            {words2.map((w, i) => (
              <span key={i} style={{ display: 'inline-block', background: i === 1 ? 'linear-gradient(90deg,#FF6B35,#E91E8C)' : 'none', WebkitBackgroundClip: i === 1 ? 'text' : 'unset', WebkitTextFillColor: i === 1 ? 'transparent' : '#F5F0E8', backgroundClip: i === 1 ? 'text' : 'unset', opacity: phase >= 1 ? 1 : 0, transform: phase >= 1 ? 'translateY(0)' : 'translateY(70px)', transition: `opacity 0.6s cubic-bezier(0.16, 1, 0.3, 1) ${0.3 + i * 0.1}s, transform 0.6s cubic-bezier(0.16, 1, 0.3, 1) ${0.3 + i * 0.1}s` }}>{w}</span>
            ))}
          </div>
        </h1>
        ...
        <div style={{ display: 'flex', gap: 0, marginTop: 'clamp(32px, 6vh, 60px)', flexWrap: 'wrap', opacity: phase >= 3 ? 1 : 0, transform: phase >= 3 ? 'translateX(0)' : 'translateX(-50px)', transition: 'opacity 0.5s cubic-bezier(0.16, 1, 0.3, 1) 0.16s, transform 0.5s cubic-bezier(0.16, 1, 0.3, 1) 0.16s' }}>
          {[
            ...
            <div key={i} style={{ flex: '1 1 clamp(100px, 30vw, 140px)', padding: '0 16px', borderRight: i < 2 ? '1px solid rgba(201,168,76,0.12)' : 'none', paddingLeft: i === 0 ? 0 : 16 }}>
              <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 'clamp(24px, 4vh, 32px)', fontWeight: 700, color: '#C9A84C', letterSpacing: '-0.02em', marginBottom: 4 }}>{s.num}</div>
              <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: 'clamp(13px, 2.2vh, 15px)', color: '#555550', lineHeight: 1.3 }}>{s.text}</div>
            </div>
          ))}
        </div>
      </div>
