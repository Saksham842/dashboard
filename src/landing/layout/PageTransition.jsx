...
// ─── PageTransition ───────────────────────────────────────────────────────────
// Gold curtain wipe transition triggered by window.triggerPageTransition(id).
// Scrolls to the target section ID while the screen is fully covered.
export const PageTransition = () => {
  ...
  React.useEffect(() => {
    if (state === 'wiping-in') {
      const reflow = setTimeout(() => { setY('0%'); setSkew('0deg'); }, 50);
      const coveredTimer = setTimeout(() => { setState('covered'); }, 550);
      ...
      const reflow = setTimeout(() => { setY('-100%'); setSkew('-6deg'); }, 50);
      const idleTimer = setTimeout(() => { setState('idle'); }, 550);
